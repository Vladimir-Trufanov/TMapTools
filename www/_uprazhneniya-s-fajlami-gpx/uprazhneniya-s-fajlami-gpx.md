...
Исправлена раскладка клавиатуры с «

### [Показать пример вывода точек трека в phpGPX](#) 

Пример вывода точек трека с использованием библиотеки phpGPX может выглядеть следующим образом. Эта библиотека позволяет читать и создавать GPX-файлы, а также извлекать данные о треках и их сегментах. 
...
foreach ($file->tracks as $track)
{
    echo "Трек: " . htmlspecialchars($track->name) . PHP_EOL;

    // Вывод точек трека
    foreach ($track->segments as $segment) 
    {
        echo "  Сегмент: " . htmlspecialchars($segment->name) . PHP_EOL;
        foreach ($segment->points as $point) 
        {
            echo "    Точка: " . 
                  htmlspecialchars($point->latitude) . ", " . 
                  htmlspecialchars($point->longitude) . ", " . 
                  htmlspecialchars($point->time) . PHP_EOL;
        }
    }
}
```
...
- Загрузка файла. Используется объект phpGPX и метод load() для чтения GPX-файла.

- Перебор треков. В цикле foreach перебираются все треки в файле.

- Доступ к точкам трека. Каждая точка трека представлена объектом Point (наследник класса Point). В примере выводятся имя трека, имя сегмента и координаты точки.

- Форматирование вывода. htmlspecialchars() используется для безопасного вывода специальных символов.

***Дополнительные возможности:***

- Извлечение статистики. Можно использовать метод stats->toArray() для получения статистических данных о треке или сегменте. 

- Создание нового GPX-файла. Библиотека позволяет создавать новые файлы с заданными данными, добавляя точки трека, сегменты и другие элементы. 

Для установки phpGPX через Composer выполните команду:
composer require sibyx/phpgpx:1.3.0